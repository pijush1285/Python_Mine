...
"""
 ...
 To run the script use the (spyder) command given below:
    run Simulation_v2.py FirstSeq.fasta 10 15
    10 -> Number of Mutation
    15 -> k-mer size
 To run the script in this codeblock use the command below:
    python .\Simulation_v3.py FirstSeq_0to200.fasta 10 15
"""
...
def rand_mutation(number_ofMutation, seq):
    sample_list = []
    #https://pynative.com/python-random-sample/
    sample_list = random.sample(range(0, len(seq)), number_ofMutation)
    return sample_list
...
def main(fasta_name, number_ofMutation, kmerSize):
     ...
     error_list=rand_mutation(number_ofMutation, seq)
     ...
     new_seq = mutseq
     new_id=str(header)
     new_rec = SeqRecord(Seq(new_seq),id=new_id, description="New_MutatedSeq_0:200")
     SeqIO.write(new_rec,"Mutated.fasta","fasta")
