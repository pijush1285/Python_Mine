#! /usr/bin/env python3
# To run this script here in code block use the code given below.
# python ./dna-seed-sim-New.py -m20 -M20 -s6 --seeds FirstSeq_0to200.fasta
    ...
    for r in alphabetReducers:
        ...
        for b in pos:
            ...
            if e <= len(seq) and all(i[j] != 255 for i, j in zip(r, seq[b:e])):
                if args.seeds:
                    seedSequence = "".join(alphabet[i] for i in seq[b:e])
                    print(b, seedSequence, sep="\t")
                else:
                    yield b
...
def main(args):
    ...
    if (args.distances or args.seeds or args.seqFile) and not args.seqFile2:
        if args.seqFile:
...
if __name__ == "__main__":
    ...
    descr = "Assess alignment seeding methods on DNA sequences."
    ...
    ap.add_argument("--seeds", action="store_true",
                    help="show seed sequences & start coords")
